...
def pattern_rainbow(t: float) -> np.ndarray:
    ...
    X, Y = np.meshgrid(x, y)
    R = np.uint8((np.sin(8 * X + t) + 1) * 127)
    G = np.uint8((np.sin(8 * Y + t * 1.5) + 1) * 127)
    B = np.uint8((np.sin(8 * (X + Y) + t * 0.7) + 1) * 127)
    return np.stack([R, G, B], axis=2)
...
# HUD
def draw_hud(surface, font, mode, slices, speed, auto, fade_enabled, fade_progress):
    lines = [
...
# KALEIDOSCOPE DRAW
def draw_kaleidoscope(target, src, angle, slices):
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if event.key in (pygame.K_ESCAPE, pygame.K_q):
                    ...
                    path = os.path.join(SCREENSHOT_DIR, f"kaleid_{
